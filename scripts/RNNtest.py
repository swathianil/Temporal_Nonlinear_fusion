...
for seed in seeds:
    ...
    # Generate labels
    y_train, A_train, V_train = torch.tensor((full_trials_train.M + 1) // 2), full_trials_train.A, full_trials_train.V
    print(y_train)
    test_acc_dict = {}
    ...
    def model(input_data, rnn_layer, linear_layer):
        ...
        out, _ = rnn_layer(input_data, h0)
        
        # Use only the last output of the RNN!
        last_out = out[-1]  # Shape: [batchsize, nb_hidden]
        
        # Apply the linear layer to the last output
        prediction = linear_layer(last_out)
    ...
    for kk, full_trials_test in enumerate(full_trials_test_list):
        test_k = kk+1
        y_test, A_test, V_test = torch.tensor((full_trials_test.M + 1) // 2), full_trials_test.A, full_trials_test.V
